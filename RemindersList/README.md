...
## Section 58: Read all reminders
    В этой секции будем использовать async func которую создали ранее, чтобы получить все reminders из event kit, так же будем конвертировать данные reminder из  event kit в данные нашего приложения
    Создаим еще один case для ошибок, в случае, если пользователь откажет предоставлении доступа к своим заметкам
    1. В файле с шибками добавим case accessDenied
    2. В файле ReminderStore создадим функцию readAll
    3. Сделаем проверку guard на случай если пользователь не предоставил доступ к заметкам
    4. Создадим константу predicate чтобы сузить количество пулучаемых reminder, так же это нужно для вызова функции по получения reminder
    5. Создадим константу в которой переведём [EKReminder] в [Remionder] c помощью map
    6. Добавим do catch блок для конвертации данных
    7. Сделаем return reminders
    
## Section 59: Request access to reminder data
    Пользователь может управлять доступом приложений к его информации. В этой главе рассмотрим как сделать запрос на доступ к информации пользователя
    1. В файле с ошибками добавим два case: .accessRestricted .unknown
    Далее добавим функцию для обработки этих ошибок
    2. В файле ReminderStore.swift сделаем функцию requestAccess
    3. Создадим константу, которая будет хранить данные о доступе
    4. Функция будет запрашивать доступ если он не предоставлен, не будет ничего делать если доступ предоставлен, и выбрасывать ошибку при других условиях
    5. Реализуем вышесказанное через switch
    6. В случае полного доступа сделаем return
